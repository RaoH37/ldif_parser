...
class TestLdifParser < Minitest::Test
  ...
  #   result = LdifParser.parse(BIG_LDIF_PATH)
  #   assert result.is_a?(Array)
  #   assert result.any?
  # end
  #
  # def test_parse_big_ldif_with_only
  #   result = LdifParser.parse(BIG_LDIF_PATH, only: %w[displayName mail sn])
  #   uniq_keys = result.map(&:keys).flatten.uniq
  #   assert (uniq_keys - %i[mail sn dn displayName]).empty?
  # end
  #
  # def test_parse_big_ldif_with_except
  #   result = LdifParser.parse(BIG_LDIF_PATH, except: %w[mail])
  #   uniq_keys = result.map(&:keys).flatten.uniq
  #   refute uniq_keys.include?(:mail)
  # end
  #
  # def test_parse_big_ldif_with_minimized
  #   result = LdifParser.parse(BIG_LDIF_PATH, minimized: true, only: %w[displayName mail sn])
